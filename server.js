...
const pool = require("./db"); // ✅ DB connection from db.js
...
app.post("/api/scores", ensureAuth, async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(`SELECT * FROM scores WHERE id = ?`, [
      result.insertId,
    ]);
...
app.post("/api/planner", ensureAuth, async (req, res) => {
  try {
    const { date, time, subject, task, category, priority, status, notes } =
      req.body;
    if (!date || !task) {
    ...
    const [rows] = await pool.query(`SELECT * FROM planner WHERE id = ?`, [
      result.insertId,
    ]);
...
app.delete("/api/planner/:id", ensureAuth, async (req, res) => {
  try {
    const { id } = req.params;
    await pool.query(
      `DELETE FROM planner WHERE id = ? AND student_id = ?`,
      [id, req.session.student.id]
    );
    res.json({ ok: true });
  } catch (err) {
...
app.post("/api/chat", async (req, res) => {
  ...
  try {
    if (process.env.GROQ_API_KEY) {
      const response = await axios.post(
        "https://api.groq.com/openai/v1/chat/completions",
        {
          ...
          messages: [
            {
              role: "system",
              content: "You are EduTrack's helpful academic assistant.",
            },
      ...
      const reply =
        response.data.choices?.[0]?.message?.content || "⚠️ No reply from Groq";
...
app.post("/diary/delete/:id", ensureAuth, async (req, res) => {
  const { id } = req.params;
  await pool.query("DELETE FROM diary WHERE id = ? AND student_id = ?", [
    id,
    req.session.student.id,
  ]);
  res.redirect("/diary");
